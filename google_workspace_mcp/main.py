...
def main():
    ...
    # Redact client secret (same fallbacks as auth.oauth_config)
    raw_id = os.getenv("GOOGLE_OAUTH_CLIENT_ID") or os.getenv("GOOGLE_CLIENT_ID") or ""
    client_secret = (
        os.getenv("GOOGLE_OAUTH_CLIENT_SECRET") or os.getenv("GOOGLE_CLIENT_SECRET") or ""
    )
    display_id = raw_id if raw_id else "Not Set"
    redacted_secret = (
        f"{client_secret[:4]}...{client_secret[-4:]}"
        if len(client_secret) > 8
        else ("(not set — set GOOGLE_CLIENT_SECRET or GOOGLE_OAUTH_CLIENT_SECRET)" if not client_secret else "Invalid or too short")
    )
    ...
    config_vars = {
        "GOOGLE_CLIENT_ID (effective)": display_id,
        "GOOGLE_CLIENT_SECRET (effective)": redacted_secret,
