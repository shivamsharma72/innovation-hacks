...
def load_client_secrets_from_env() -> Optional[Dict[str, Any]]:
    ...
    Environment variables used:
        - GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET (preferred), or
        - GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET (same as gateway web app)
    ...
    client_secret = os.getenv("GOOGLE_OAUTH_CLIENT_SECRET") or os.getenv(
...
async def build_google_authorization_url(
    ...
    """
    Build the full Google OAuth URL and persist PKCE state + verifier.

    Use this for HTTP 302 redirects so chat UIs cannot truncate the query string
    (truncation causes Google's "Required parameter is missing: response_type").
    """
...
async def start_auth_flow(
    ...
    try:
        auth_url = await build_google_authorization_url(
            user_google_email=user_google_email,
            redirect_uri=redirect_uri,
        )
