...
async def workspace_mcp_home(_request: Request):
    """Browser help: / is not a login page; Google OAuth is started from chat or MCP tools."""
    try:
        version = metadata.version("workspace-mcp")
    except metadata.PackageNotFoundError:
        version = "dev"
    ext = (os.getenv("WORKSPACE_EXTERNAL_URL") or "").strip() or "http://localhost:8002"
    default_email = (os.getenv("USER_GOOGLE_EMAIL") or "").strip()
    email_hint = (
        f"<p><strong>USER_GOOGLE_EMAIL</strong> is set to <code>{default_email}</code> "
        "(tools can default to this account).</p>"
        if default_email
        else "<p>Set <strong>USER_GOOGLE_EMAIL</strong> in your env to your Google address "
        "so tools don’t always need an explicit email.</p>"
    )
    html = f"""<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"/><title>Workspace MCP</title>
<style>body{{font-family:system-ui,sans-serif;max-width:42rem;margin:2rem auto;padding:0 1rem;line-height:1.5;color:#e4e4e7;background:#18181b}}
code{{background:#27272a;padding:0 .2rem;border-radius:4px}} a{{color:#a5b4fc}}</style>
</head>
<body>
<h1>Google Workspace MCP</h1>
<p>This port is the <strong>MCP server</strong> (Calendar, Gmail, Drive, …). The JSON you saw on <code>/health</code> only means the process is up — <strong>there is no login form here</strong>.</p>
<h2>Not the same as your web app</h2>
<p>If you see <strong>Auth0</strong> or “Sign in with Google” on <code>http://localhost:3000</code>, that is only for the <strong>Next.js app → gateway API</strong>. It does <strong>not</strong> grant Gmail/Calendar to this MCP server.</p>
<h2>How to sign in to Google for MCP</h2>
<p><strong>Recommended</strong> — avoids chat apps truncating long Google URLs (which causes Google error “missing response_type”):<br/>
<a href="/oauth/google/authorize" style="display:inline-block;margin-top:.5rem;padding:.5rem 1rem;background:#4f46e5;color:white;border-radius:8px;text-decoration:none;font-weight:600">Sign in with Google</a>
<span style="opacity:.85"> (uses <code>USER_GOOGLE_EMAIL</code> or add <code>?email=you@gmail.com</code>)</span></p>
<ol>
<li>In <strong>chat</strong>, ask for something that uses Google (e.g. list calendar). If you use the long <code>accounts.google.com/…</code> link from the assistant, copy the <strong>entire</strong> URL — broken links cause OAuth errors.</li>
<li>Or use <a href="/oauth/google/authorize">/oauth/google/authorize</a> above, then return to chat and retry.</li>
</ol>
{email_hint}
<p>MCP JSON-RPC endpoint: <code>{ext}/mcp</code> · Health JSON: <a href="/health">/health</a> · Version {version}</p>
</body></html>"""
    return HTMLResponse(html)


@server.custom_route("/oauth/google/authorize", methods=["GET"])
async def oauth_google_authorize_redirect(request: Request):
    """
    Short entry point → HTTP 302 to Google with a full, valid OAuth query string.

    Chat UIs often truncate megabyte-long auth URLs; opening a partial URL yields
    Google's 400 invalid_request (missing response_type).
    """
    if is_oauth21_enabled():
        return HTMLResponse(
            content="<p>OAuth 2.1 mode: authenticate via your MCP client.</p>",
            status_code=400,
        )
    err = check_client_secrets()
    if err:
        return create_server_error_response(err)

    email = (
        (request.query_params.get("email") or "").strip()
        or (os.getenv("USER_GOOGLE_EMAIL") or "").strip()
    )
    if not email:
        return HTMLResponse(
            content="""<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"/><title>Email required</title></head>
<body style="font-family:system-ui;max-width:32rem;margin:2rem;padding:0 1rem">
<p>Set <code>USER_GOOGLE_EMAIL</code> in the server environment, or open:</p>
<p><code>/oauth/google/authorize?email=your.address@gmail.com</code></p>
<p><a href="/">← Home</a></p>
</body></html>""",
            status_code=400,
        )
    ...
    try:
        ...
        auth_url = await build_google_authorization_url(email, redirect_uri)
        return RedirectResponse(url=auth_url, status_code=302)
    except Exception as e:
        logger.exception("OAuth authorize redirect failed")
...
async def legacy_oauth2_callback(request: Request) -> HTMLResponse:
    ...
    if not code:
        msg = (
            "This page only works after Google redirects you here with a ?code=… parameter. "
            "If you opened http://localhost:8002/oauth2callback manually or bookmarked it, "
            "that will always fail. Start from the long "
            "<strong>https://accounts.google.com/o/oauth2/auth?…</strong> link shown in chat "
            "(or run a calendar/Gmail request again and use the new link). "
            "In Google Cloud Console, add authorized redirect URI: "
            "<code>http://localhost:8002/oauth2callback</code>."
        )
        logger.error(
            "OAuth callback hit without ?code= (user likely opened /oauth2callback directly)"
        )
        return HTMLResponse(
            content=f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"/><title>OAuth — wrong entry</title>
<style>
body{{font-family:system-ui,sans-serif;max-width:36rem;margin:2rem auto;padding:0 1rem;line-height:1.5;color:#e4e4e7;background:#18181b}}
code{{background:#27272a;padding:.1rem .3rem;border-radius:4px;font-size:.9em}}
a{{color:#a5b4fc}}
</style></head><body>
<h1>Don’t open this URL directly</h1>
<p>{msg}</p>
<p><a href="/">← Workspace MCP home</a></p>
</body></html>""",
            status_code=400,
        )
